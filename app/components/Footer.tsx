  ...
  return (
    <footer className="siteFooter">
      <div className="siteFooterInner">
        <div className="footerBrand">
          <Image
            ...
            height={88}
          />
          <p>{legalBusinessName} develops and operates digital products and apps.</p>
        </div>

        <div>
          <p className="footerLabel">Legal &amp; Policies</p>
          <nav className="footerLinks" aria-label="Legal policies">
            {policies.map((policy) => (
              <Link key={policy.slug} href={`/legal/${policy.slug}`}>
                {policy.title}
              </Link>
            ))}
          </nav>
      ...
      <div className="legalBar">
        ...
        <p>© 2026 {legalBusinessName}. All rights reserved.</p>
      </div>
    </footer>
  );
}
